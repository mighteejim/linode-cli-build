"""BuildWatch Service - Real-time container monitoring and event tracking.

This module provides the BuildWatch service script that runs on deployed instances
to monitor Docker containers, track events, detect issues, and provide HTTP API access.
"""

from __future__ import annotations


# Complete Python script for BuildWatch service
BUILDWATCH_SERVICE_SCRIPT = r'''#!/usr/bin/env python3
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                if self.running:
                    log_error("Docker events process died, restarting in 5s...")
                    time.sleep(5)
...
def get_build_watcher_systemd_unit(deployment_id: str, app_name: str) -> str:
    """Generate systemd unit file for BuildWatch service.
...
# Environment
Environment="BUILD_DEPLOYMENT_ID={deployment_id}"
Environment="BUILD_APP_NAME={app_name}"
...
def get_logrotate_config() -> str:
    """Generate logrotate configuration for BuildWatch logs.
    
    Returns:
        Logrotate configuration content
    """
    return """/var/log/build-watcher/*.log {
    daily
    rotate 7
    compress
    delaycompress
...
def get_installation_commands(deployment_id: str, app_name: str) -> list[str]:
    """Generate shell commands to install BuildWatch service.
    ...
    return [
        ...
        "cat > /usr/local/bin/build-watcher << 'BUILDWATCH_EOF'\n" + 
        BUILDWATCH_SERVICE_SCRIPT + 
        "\nBUILDWATCH_EOF",
        "chmod +x /usr/local/bin/build-watcher",
        
        # Install systemd unit
        "cat > /etc/systemd/system/build-watcher.service << 'SYSTEMD_EOF'\n" +
        get_build_watcher_systemd_unit(deployment_id, app_name) +
        "\nSYSTEMD_EOF",
        
        # Install logrotate config
        "cat > /etc/logrotate.d/build-watcher << 'LOGROTATE_EOF'\n" +
        ...
        "systemctl is-active --quiet build-watcher && echo '✓ BuildWatch service started' || echo '✗ BuildWatch failed to start'"
    ]
